...
/**
 * Middleware para validar a presença do parâmetro 'dis_id' em rotas que precisam de um ID de Disciplina.
 *
 * Este middleware verifica se a rota existe e se o parâmetro obrigatório 'dis_id' foi informado.
...
class ValidateDisciplinaId implements MiddlewareInterface
{
     ...
     * @throws ErrorResponse  Se a rota ou o parâmetro 'dis_id' estiverem ausentes
     */
    public function process(Request $request, RequestHandler $handler): Response
    {
        ...
        // Recupera os argumentos/parametros passados na rota (ex: /Disciplinas/{dis_id})
        $routeArgs = $route->getArguments();

        // -----------------------------------------------------------
        // Validação 2: verificar se o parâmetro obrigatório 'dis_id' existe e não está vazio
        // -----------------------------------------------------------
        if (!isset($routeArgs['dis_id']) || $routeArgs['dis_id'] === "") {
            throw new ErrorResponse(
                httpCode: 400,
                message: "Erro na validação de dados",
                error: [
                    "message" => "O parâmetro 'dis_id' é obrigatório!"  // Mensagem detalhada
                ]
            );
        }
